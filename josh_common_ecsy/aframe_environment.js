...
import {Mesh, SphereBufferGeometry, MeshLambertMaterial, BackSide, Color, AmbientLight, AdditiveBlending, NormalBlending, Vector3 } from "https://threejs.org/build/three.module.js"
...
export class AframeEnvironment {
    constructor() {
        this.preset = "default"
    }
}
export class AframeEnvironmentSystem extends System {
    init() {
        console.log("initing")
        ...
        for (let i in this.assets){
            for (let j = 0; j < this.assets[i].length; j++) {
                const asset = this.assets[i][j]
                if (asset.type !== 'mesh') continue;
                for (let v = 0, len = asset.vertices.length; v < len; v++) {
    ...
    execute() {
        this.queries.envs.added.forEach(ent => {
            console.log("added an env")
            const env = ent.getMutableComponent(AframeEnvironment)
            env.data = presets[env.preset]
            if(!env.data) env.data = presets.default
            console.log("data",env.data)
            const skybox = new Mesh(
                new SphereBufferGeometry(this.STAGE_SIZE), //TODO: use theta start
...
AframeEnvironmentSystem.queries = {
    three: {
        components: [ThreeCore]
    },
    envs: {
        components:[AframeEnvironment],
        listen: {
            added:true,
        }
    }
